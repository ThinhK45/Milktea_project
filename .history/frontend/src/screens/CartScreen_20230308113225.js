...
const CartScreen = ({ match, location, history }) => {
    ...
    const qty = location.search
        ...
        : 1;
    const sizeProduct = location.search
        ? Number(location.search.split('=')[1].split('&')[1].split('=')[1])
        : 1;
    ...
    useEffect(() => {
        if (productId) {
            dispatch(addToCart(productId, qty, sizeProduct));
        }
    }, [dispatch, productId, qty, sizeProduct]);
