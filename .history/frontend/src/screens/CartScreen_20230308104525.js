...
const CartScreen = ({ match, location, history }) => {
    ...
    const qty = location.search ? Number(location.search.split('=')[1]) : 2;
    const size = location.search ? Number(location.search.split('=')[1]) : 2;

    const cart = useSelector((state) => state.cart);
    const { cartItems } = cart;
    // const [size, setSize] = useState(cartItems.size);
